...
class FavorecidoPDF extends FPDF
{
    ...
    function listagem()
    {
        try {
            ...
            // Cabeçalho
            $this->Cell(30, 7, iconv("utf-8", "cp1252", $cabecalho[0]), 1); 
            $this->Cell(160, 7, iconv("utf-8", "cp1252", $cabecalho[1]), 1);    
            $this->Ln();

            // Dados
            foreach ($dados as $linha) {                
                $this->Cell(30, 6, iconv("utf-8", "cp1252", $linha['id']), 1);                
                
                $this->Cell(160, 6, iconv("utf-8", "cp1252", $linha['nome']), 1);
