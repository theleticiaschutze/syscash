...
function validaDados($registro)
{
    ...
    if (!htmlspecialchars(strip_tags(filter_var($registro->descricao_contapagar, FILTER_UNSAFE_RAW)), ENT_QUOTES, 'UTF-8')) {
        $erros["descricao_contapagar"] =  "Descrição: Campo vazio e ou informação inválida!";
    }

    if (!htmlspecialchars(strip_tags(filter_var($registro->favorecido_id_contapagar, FILTER_UNSAFE_RAW)), ENT_QUOTES, 'UTF-8')) {
    ...
    if (!htmlspecialchars(strip_tags(filter_var($registro->datavencimento_contapagar, FILTER_UNSAFE_RAW)), ENT_QUOTES, 'UTF-8')) {
        $erros["datavencimento_contapagar"] =  "Data Vencimento: Campo vazio e ou informação inválida!";
    }

    if (!htmlspecialchars(strip_tags(filter_var($registro->categoria_id_contapagar, FILTER_UNSAFE_RAW)), ENT_QUOTES, 'UTF-8')) {
