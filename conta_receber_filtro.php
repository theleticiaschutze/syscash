...
function validaDados($registro)
{
    ...
    if (!htmlspecialchars(strip_tags(filter_var($registro->descricao_contareceber, FILTER_UNSAFE_RAW)), ENT_QUOTES, 'UTF-8')) {
        $erros["descricao_contareceber"] =  "Descrição: Campo vazio e ou informação inválida!";
    }

    if (!htmlspecialchars(strip_tags(filter_var($registro->favorecido_id_contareceber, FILTER_UNSAFE_RAW)), ENT_QUOTES, 'UTF-8')) {
    ...
    if (!htmlspecialchars(strip_tags(filter_var($registro->datavencimento_contareceber, FILTER_UNSAFE_RAW)), ENT_QUOTES, 'UTF-8')) {
        $erros["datavencimento_contareceber"] =  "Data Vencimento: Campo vazio e ou informação inválida!";
    }

    if (!htmlspecialchars(strip_tags(filter_var($registro->categoria_id_contareceber, FILTER_UNSAFE_RAW)), ENT_QUOTES, 'UTF-8')) {
